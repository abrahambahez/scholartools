...
def test_refs_add_valid_json(capsys):
    ...
    with patch("scholartools.add_reference", return_value=result) as mock_add:
...
def test_refs_get_calls_get_reference(capsys):
    ...
    with patch("scholartools.get_reference", return_value=result) as mock_get:
        with pytest.raises(SystemExit) as exc_info:
            _run(["refs", "get", "key1"])
        assert exc_info.value.code == 0
...
def test_refs_update_valid_json(capsys):
    ...
    with patch("scholartools.update_reference", return_value=result) as mock_update:
        with pytest.raises(SystemExit) as exc_info:
...
def test_refs_rename_calls_rename_reference():
    ...
    with patch("scholartools.rename_reference", return_value=result) as mock_rename:
        with pytest.raises(SystemExit) as exc_info:
...
def test_refs_delete_calls_delete_reference():
    ...
    with patch("scholartools.delete_reference", return_value=result) as mock_del:
        with pytest.raises(SystemExit) as exc_info:
...
def test_refs_list_json_output(capsys):
    ...
    with patch("scholartools.list_references", return_value=result) as mock_list:
        with pytest.raises(SystemExit) as exc_info:
            _run(["refs", "list"])
        assert exc_info.value.code == 0
...
def test_refs_list_plain_output(capsys):
    rows = [ReferenceRow(citekey="k1", title="Title", authors="Auth", year=2020)]
    result = ListResult(references=rows, total=1, page=1)
    with patch("scholartools.list_references", return_value=result):
        with pytest.raises(SystemExit) as exc_info:
...
def test_refs_filter_all_args(capsys):
    ...
    result = ListResult(references=rows, total=1, page=1)
    with patch("scholartools.filter_references", return_value=result) as mock_filter:
        with pytest.raises(SystemExit) as exc_info:
            _run(
                [
                    "refs",
...
def test_refs_filter_plain_output(capsys):
    ...
    result = ListResult(references=rows, total=1, page=1)
    with patch("scholartools.filter_references", return_value=result):
        with pytest.raises(SystemExit) as exc_info:
